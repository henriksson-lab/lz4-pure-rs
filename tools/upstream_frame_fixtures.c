...
static void print_frame_hash_fixture_with_block(
    const char* name,
    ...
    LZ4F_preferences_t prefs = {0};
    prefs.frameInfo.blockSizeID = block_size;
    prefs.frameInfo.blockMode = LZ4F_blockIndependent;
    prefs.frameInfo.contentChecksumFlag = LZ4F_contentChecksumEnabled;
    prefs.frameInfo.blockChecksumFlag = LZ4F_noBlockChecksum;
    prefs.compressionLevel = level;
...
static void print_frame_hash_fixture(const char* name, const unsigned char* input, size_t input_len, int level) {
    print_frame_hash_fixture_with_block(name, input, input_len, level, LZ4F_max64KB);
}
...
static void print_dict_id_fixture(void) {
    const char* input = "frame with upstream dict id";
    LZ4F_preferences_t prefs = {0};
    prefs.frameInfo.blockSizeID = LZ4F_max64KB;
    prefs.frameInfo.blockMode = LZ4F_blockIndependent;
    prefs.frameInfo.contentChecksumFlag = LZ4F_contentChecksumEnabled;
    prefs.frameInfo.blockChecksumFlag = LZ4F_noBlockChecksum;
    prefs.frameInfo.contentSize = strlen(input);
    prefs.frameInfo.dictID = 0xdecafbadU;
    prefs.compressionLevel = 0;

    size_t bound = LZ4F_compressFrameBound(strlen(input), &prefs);
    unsigned char* output = malloc(bound);
    if (output == NULL) {
        exit(2);
    }

    size_t written = LZ4F_compressFrame(output, bound, input, strlen(input), &prefs);
    if (LZ4F_isError(written)) {
        fprintf(stderr, "%s\n", LZ4F_getErrorName(written));
        free(output);
        exit(1);
...
int main(void) {
    ...
    const size_t huge_len = 8 * 1024 * 1024;
    unsigned char* huge = malloc(huge_len);
    if (huge == NULL) {
        return 2;
    }
    fill_pattern(huge, huge_len);
    ...
    print_frame_hash_fixture("hc9_frame_patterned_8mib", huge, huge_len, 9);
    print_frame_hash_fixture_with_block("hc9_frame_patterned_8mib_4mb", huge, huge_len, 9, LZ4F_max4MB);
    print_cdict_fixture();
    print_dict_id_fixture();
    free(huge);
    free(large);
    return 0;
}
